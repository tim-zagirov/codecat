...
`make bundle` and **`make sign` both run clean**. The signed bundle at
`dist/CodeCat.app` carries `TeamIdentifier=T9N3G9LHXL`, the hardened runtime
flag, a trusted timestamp, the Apple-events entitlement inside the signature,
and a separately signed `codecat-hook`. `codesign --verify --deep --strict`
passes; `spctl` reports `source=Unnotarized Developer ID`, the correct verdict
for a bundle that has not been through the notary service.

The first attempt hung with no output at all — that was the macOS keychain
dialog asking whether `codesign` may use the private key, waiting behind another
window. Answered once, later runs do not ask.

`make notarize` and `make release` stop at the next step: `xcrun notarytool
history --keychain-profile codecat` reports *No Keychain password item found for
profile*. Creating it needs an app-specific password from appleid.apple.com, so
it has to be done by a human.
