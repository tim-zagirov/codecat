...
- Ветка `claude/codecat-mascot-skins-spec-b6a4de` (облики котика), **239 тестов зелёные** (`swift test`), рабочее дерево чистое. В `master` она на момент записи не влита.
...
- Девять обликов: нарисованный кот (по умолчанию) + шесть котов LuizMelo + Elthen + mxmaze.
- Переключение — сетка живых превью 5×2 в панели деталей, там же раскрывающаяся строка
...
Плавность анимаций, перетаскивание котика и сохранение позиции между запусками, отклик переключателей в панели деталей, сводка «пока тебя не было» после разблокировки экрана, весь переход к сессии (пункты 9–12) и весь переключатель обликов (пункты 13–16). Спрайтовые облики проверены только рендером в PNG — все 45 сочетаний «облик × состояние» отрисованы и просмотрены, но в живом приложении их никто не видел.
