...
23. Включить тумблер «Прятать остров, когда сессий нет» при нуле сессий → остров исчезает; запустить сессию → остров возвращается сам, без клика по меню.
...
28. В меню-баре видна версия — «CodeCat 0.2.0 (136)», и число в скобках совпадает с `git rev-list --count HEAD` той сборки, что стоит.
